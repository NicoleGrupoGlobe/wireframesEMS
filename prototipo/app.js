...
function span(n){return 'style="grid-column:span '+(n||12)+'"';}
...
function renderBlock(b){
  ...
  switch(b.type){
    ...
    case "actions": return '<div class="bare" '+span(b.colspan)+'><div class="actions">'+(b.btns||["Acción"]).map(function(x,i){return '<button class="btn'+(i===0?' btn-primary':'')+'" data-act="btn" data-v="'+esc(x)+'">'+esc(x)+'</button>';}).join("")+'</div></div>';
    case "tabs": return '<div class="bare" '+span(b.colspan)+'><div class="tabs">'+(b.tabs||["Tab"]).map(function(x,i){return '<div class="tab'+(i===(b.active||0)?' active':'')+'" data-act="tab">'+esc(x)+'</div>';}).join("")+'</div></div>';
    case "legend": return '<div class="bare" '+span(b.colspan||12)+'><div class="legend"><b>Semáforo:</b><span><span class="dot ok"></span>Normal</span><span><span class="dot warn"></span>Alerta</span><span><span class="dot crit"></span>Crítico</span><span><span class="dot null"></span>Sin dato</span></div></div>';
    default: inner=head(b)+((b.subs&&b.subs.length)?('<ul class="plist">'+b.subs.map(function(x){return '<li>'+esc(x)+'</li>';}).join("")+'</ul>'):placeholder(b));
  }
  return '<div class="card pad-b" '+span(b.colspan)+'>'+inner+reqs(b)+'</div>';
}
